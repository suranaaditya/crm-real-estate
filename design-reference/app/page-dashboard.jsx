...
window.PageDashboard = function PageDashboard({ onNav }) {
  ...
  const visitsToday = data.visits.filter(v => v.date === "2026-04-29").length;
  const visitsWeek = data.visits.filter(v => {
    const d = new Date(v.date), t = new Date("2026-04-29");
    const diff = (d - t) / 86400000; return diff >= -3 && diff <= 7;
  }).length;
  const overdue = data.paymentDues.filter(p => p.status === "overdue").length;
  const overdueAmt = data.paymentDues.filter(p => p.status === "overdue").reduce((a, p) => a + p.amount, 0);
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px 24px", display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <Btn variant="accent" size="sm" icon="plus" onClick={() => onNav("leads")}>New Lead</Btn>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14 }}>
        <StatCard label="OPEN LEADS" value={data.leads.filter(l => !["booked", "lost"].includes(l.stage)).length} delta="+12 this week" icon="users" />
        <StatCard label="VISITS THIS WEEK" value={visitsWeek} delta="+3 vs last" icon="calendar" />
        <StatCard label="BOOKINGS APRIL" value={data.bookings.length} delta={fmtINR(data.bookings.reduce((a, b) => a + b.bsp, 0))} icon="rupee" />
        <StatCard label="COLLECTIONS PENDING" value={fmtINR(overdueAmt)} delta={overdue + " overdue"} deltaTone="down" icon="money" />
            ...
            <div key={p.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "var(--neutral-400)", marginTop: 2 }}>{p.locality} · {p.typology}</div>
                </div>
          ...
          {recent.map((l, i) => (
            <div key={l.id} style={{
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: "var(--neutral-400)" }}>{l.interest} at {l.projectName} · {l.source}</div>
              </div>
