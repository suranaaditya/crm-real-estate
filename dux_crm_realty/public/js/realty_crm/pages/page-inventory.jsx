    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {hoverUnit && (
        <div style={{
          ...
          <Btn variant="primary" size="sm" style={{ width: "100%" }} onClick={async () => {
            if (hoverUnit.status === "sold") { frappe.show_alert("Unit " + hoverUnit.tower + "-" + hoverUnit.num + " is sold."); return; }
            try {
              const r = await frappe.call({ method: "dux_crm_realty.api.crm.block_unit", args: { unit_id: hoverUnit.id } });
              frappe.show_alert({ message: "Unit " + hoverUnit.id + " is now " + r.message.status, indicator: "orange" });
              setHoverUnit(null);
              if (window.__refreshCRM) await window.__refreshCRM();
            } catch (e) { frappe.msgprint(e.message || "Could not update unit"); }
          }}>
            {hoverUnit.status === "available" ? "Block / Hold unit" : hoverUnit.status === "blocked" ? "Release hold" : "View details"}
          </Btn>
        </div>
      )}
